...
// 		// while(
// 		getline(infile, temp); // get the whole line, place in temp
		
// 			// Parse string, place individual items in proper position
// 			infile >> myProcess[i].PROCESS_ID >> myProcess[i].ACTION;
// 			if(temp == "\0") {
// 				myProcess[i].PAGE = 0;
// 			}
// 			else {
// 				infile >> myProcess[i].PAGE;
// 			 // myProcess[i].PAGE;
// 			}
			
// 			cout << myProcess[i].PROCESS_ID << ": " << myProcess[i].ACTION << ": " << myProcess[i].PAGE << endl;
// 			i++;
...
int main(){
    ...
    //     cout << myProcess[i].PROCESS_ID <<": ";
    //     cout << myProcess[i].ACTION << "\t";
    //     cout << myProcess[i].PAGE << "\n";
    //     i++;
    // }
    // To test that it works....
    cout << "Number of actions in memory.dat:\t " << totalActions << "\n";
...
int ReadFile(string fileName)
{
    ...
    while(!infile.eof()){
        ...
        while(check) // get the whole line, place in temp
        {
            ...
            if((myProcess[i].ACTION == "C")||(myProcess[i].ACTION == "T"))
            {
                myProcess[i].PAGE = 0;
            }
